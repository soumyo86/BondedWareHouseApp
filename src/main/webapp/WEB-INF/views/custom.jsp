...
<title>Bonded Warehouse Custom Portal</title>
...
function updateStatuss(dId, dSource, dDestination, dStatus, flag) {
    ...
    if(dStatus == "Processed"){    	
    	document.getElementById("status").innerHTML = "<option value='Verified'>Verified</option><option value='Rejected'>Rejected</option>";    	
    }
    else if(dStatus == "Verified")
    	document.getElementById("status").innerHTML = "<option value='Approved'>Approved</option><option value='Rejected'>Rejected</option>";
}  
...
function showHideReasonCode(){
    ...
    if($(options[0]).attr('value') == 'Approved'){
...
<div class="panel-heading">
	...
	<div class="panel panel-success">
		<div class="panel-heading">
			<div class="welcomeText"><h3>Welcome Custom</h3></div>
		...
		<c:if test="${param.statusUpdate == 'success' || statusUpdate == 'success'}">
			...
			<c:if test="${highpriorityDocListCount!=''}">
				<div class="alertHigh alert-info">
						<strong><img title="High Alert" style="width:20px; height:20px;" src="./resources/img/alert.gif"/><a href="./customLogin">  Documents that required your immediate attention: ${highpriorityDocListCount}</a></strong>
				</div>
			</c:if>
			<div class="container">
				<c:choose>
				...
				<table class="table table-bordered display" id="datatable">	
					...
					<tbody>
						...
						<tr>
								...
								<td>
								<c:if test="${prioritydoc.getStatus() == 'Processed' || prioritydoc.getStatus() == 'Verified' }">
					...
					<c:if test="${order.getStatus() == 'Rejected'}">
						...
						<c:if test="${order.getStatus() != 'Rejected'}">
							...
							<tr>
								...
								<td>${order.getStatus()}</td>
								<td>${order.getEstimatedTime()}</td>
								<c:if test="${order.getStatus() == 'Processed' || order.getStatus() == 'Verified'}">
									<td>
											<span id="update_button"
												class=" update_button btn" data-toggle="modal"
												data-target="#updateStatus1" title="Update Status" style="margin-top:8px;"
												onclick="updateStatuss(${order.getDocumentId()} , '${order.getSource()}' , '${order.getDestination()}' , '${order.getStatus()}' , '${order.getStatus()}')"><img style="width:30px; height:30px; float:left;" src="./resources/img/update-status.png"/></span>
									...
									<c:if test="${order.getStatus() != 'Received' && order.getStatus() != 'Processed' && order.getStatus() != 'Verified'}">
									  <td>
										<img ng-src="./resources/img/truck-search.png" title="Locate the truck" alt="Truck Track" style="width:50px; height:50px; cursor:pointer; float:right;" ng-click="initMap('${order.getTruckId()}','${order.getDocumentId()}');"/>
									  </td>
									</c:if>
							...
							<form role="form" method="post" action="updateStatus">
								...
								<div class="form-group">
									<table>
										...
										<tr>
											<td><font face="verdana" size="2px">Status:</td>
										...
										<td style="padding-top:15px;">
											...
											<select name="reasonCode" class="form-control" id="reasonDes">
											<option value="" selected="selected">Select One Reason Code</option>
											<c:forEach items="${rejections}" var="rejectionsItem">
											 	<option id="${rejectionsItem.rejection_description}" value="${rejectionsItem.rejection_description}">${rejectionsItem.rejection_description}</option>
											</c:forEach>
											</select>
										</div>
										</td>
									...
									<input type="HIDDEN" id="dStatus" value="" class="form-control" />
									<input type="hidden" id="flag" name="flag" value="" />
								<div class="modal-footer">
									<button type="button" class="btn btn-secondary"
										data-dismiss="modal">Close</button>
									<button type="submit" class="btn btn-primary" id="statusUpdateSave" value="Update" onclick="disableUpdate()">Save</button>
								</div>
							</form>
						</div>
					</div>
