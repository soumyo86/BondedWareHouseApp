...
function myFunction() {
  ...
  <div id="fade" class="black_overlay"></div>
	...
	<div class="panel panel-success">
		<div class="panel-heading" >
		    <div class="createDoc"><a href="createNewDoc"><img src="./resources/img/document_add.png" title="Create CB Document" alt="Create CB Document" style="width:40px; height:40px; cursor:pointer"/></a></div>
		...
		<c:if test="${param.statusUpdate == 'success' || statusUpdate == 'success'}">
		   ...
		   <c:if test="${highpriorityDocListCount!=''}">
		  	 <div class="alertHigh alert-info">
				<strong><img title="High Alert" style="width:20px; height:20px;" src="./resources/img/alert.gif"/><a href="./warehouseLogin">  Documents that required your immediate attention: ${highpriorityDocListCount}</a></strong>
			</div>
		  </c:if>
		<div class="container">
				<c:choose>
						...
						<c:forEach items="${documents}" var="order" varStatus="loop">
							...
							<c:if test="${loop.index == loop2.index }">
								...
								<c:if test="${order.getStatus() == 'Approved'}">
									<td>
										<span id="update_button"
											...
											onclick="updateStatuss(${order.getDocumentId()} , '${order.getSource()}' , '${order.getDestination()}' , '${order.getStatus()}', 'approved')"><img style="width:30px; height:30px; float:left;" src="./resources/img/update-status.png"/></span>
								...
								<c:if test="${order.getStatus() == 'Rejected'}">
									...
									<td>${updated}</td>
									<td>
									<c:if test="${order.getReasonCode() == 'Document Error'}" var="true">	
										<a href="updateDoc?id=${order.getDocumentId()}"><img title="Update Document" style="width:40px; height:40px; float:left; margin-top:5px;" src="./resources/img/update-doc.png"/></a>
									</c:if>
										<img ng-src="./resources/img/truck-search.png" title="Locate the truck" alt="Truck Track" style="width:50px; height:50px; cursor:pointer; float:right;" ng-click="initMap('${order.getTruckId()}','${order.getDocumentId()}');"/>
									</td>
								</c:if>
						...
						<c:if test="${order.getStatus() != 'Rejected'}">
							<c:forEach items="${documents}" var="order" varStatus="loop">
								...
								<tr>
									...
									<c:if test="${order.getStatus() == 'Approved'}">
										<td>
											<span id="update_button" class="update_button btn" data-toggle="modal" data-target="#updateStatus1" title="Update Status" style="margin-top:8px;"
												onclick="updateStatuss(${order.getDocumentId()} , '${order.getSource()}' , '${order.getDestination()}' , '${order.getStatus()}' , 'approved')"><img style="width:30px; height:30px; float:left;" src="./resources/img/update-status.png"/></span>
											<img ng-src="./resources/img/truck-search.png" title="Locate the truck" alt="Truck Track" style="width:50px; height:50px; cursor:pointer; float:right;" ng-click="initMap('${order.getTruckId()}','${order.getDocumentId()}');"/>
										</td>
									</c:if>	
									<c:if test="${order.getStatus() != 'Received' && order.getStatus() != 'Approved'}">
										<td>			
											<img ng-src="./resources/img/truck-search.png" title="Locate the truck" alt="Truck Track" style="width:50px; height:50px; cursor:pointer; float:right;" ng-click="initMap('${order.getTruckId()}','${order.getDocumentId()}');"/>	
										</td>
									</c:if>					
				 ...
				 <table class="table table-bordered display" id="datatable">
					...
					<tbody>
						...
						<tr>
								...
								<td>
								<c:if test="${prioritydoc.getStatus()=='Rejected' && prioritydoc.getReasonCode() == 'Document Error'}" var="true">
								    <a href="updateDoc?id=${prioritydoc.getDocumentId()}"><img title="Update Document" style="width:40px; height:40px; float:left; margin-top:5px;" src="./resources/img/update-doc.png"/></a>
								</c:if>
